...
Begin VB.Form frmDrvAll 
   ...
   Begin VB.CommandButton btnLoad 
      ...
      Width           =   3255
   End
   Begin MSComctlLib.ListView lstDrvAll 
      Height          =   3735
      Left            =   360
      TabIndex        =   0
      TabStop         =   0   'False
...
'справка за всички водачи - филтър по период от резултатната таблица зададен от формата
'всеки ред от справката изобразява един водач

    Me.Caption = repDrvAll
    ...
    Me.lstDrvAll.ColumnHeaders.Clear
    Me.lstDrvAll.ListItems.Clear
    
    Set colx = Me.lstDrvAll.ColumnHeaders.Add()
        colx.Text = uniCode
        colx.Width = 800
...
AgainOther:
    ...
    commR = "SELECT name_drv, time_exp_start, exp_num, km_clnt, exp_q, total_vol FROM mix_result_bc" & MachineNumber & " WHERE stamp_date >= '" & DayStart _
    & "' AND stamp_date <= '" & DayEnd _
    & "' ORDER BY name_drv, mix_num, exp_num, stamp_date ASC;"
    ...
    If Not rsR.EOF And Not rsR.BOF Then
        ...
        Set itmX = Me.lstDrvAll.ListItems.Add(rcCounter, , "")
...
beginCount:
    ...
    If Not rsR.EOF And Not rsR.BOF Then
        tempDrv = rsR!name_drv 'вземаме първия срещнат водач
    End If
    ...
    Do While Not rsR.EOF
        If tempDrv = rsR!name_drv Then 'ако записа отговаря на името - при първи обход винаги
    ...
    ind = ind + 1
    Set itmX = Me.lstDrvAll.ListItems.Add(rcCounter, , Format(ind, "0000"))
        itmX.SubItems(1) = tempDrv
...
'--------------------------End PostgreSQL-----------------------------------
    ...
    For i = rcCounter - ind + 1 To Me.lstDrvAll.ListItems.count
        tDay = tDay + Val(Me.lstDrvAll.ListItems.Item(i).SubItems(2))
        tKm = tKm + Val(Me.lstDrvAll.ListItems.Item(i).SubItems(3))
        tExp = tExp + Val(Me.lstDrvAll.ListItems.Item(i).SubItems(4))
        tExpVol = tExpVol + CSng(rDs(Me.lstDrvAll.ListItems.Item(i).SubItems(5)))
        If rQinForm = 1 Then
            tVol = tVol + CSng(rDs(Me.lstDrvAll.ListItems.Item(i).SubItems(6)))
        End If
    Next i
    ...
    If have = 2 Then
        ...
        Set itmX = Me.lstDrvAll.ListItems.Add(rcCounter, , "")
            ...
            itmX.SubItems(3) = tKmT
            itmX.SubItems(4) = tExpT
...
Private Sub btnPrint_Click()
    Call PrintLVPic(Me.lstDrvAll, 1, True, True, True, repDrvAll)
End Sub

Private Sub btnExport_Click()
    Call ExportToExcel(Me.lstDrvAll)
End Sub
