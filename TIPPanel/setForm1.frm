...
Begin VB.Form setForm1 
   ...
   Begin VB.CheckBox chPrint 
      Caption         =   "Печат на Форма 1  при потвърждението след всяка експедиция"
      Height          =   195
      Left            =   2040
      TabIndex        =   95
      Top             =   7080
...
Private Sub btnSave_Click()
    ...
    numSheetsForm1 = Me.txtNumSheets.Text
    strSubKey = Trim(PlaceProgSet1)
    PrevSet = CheckRegistryKey(HKEY_CURRENT_USER, strSubKey)
    ...
    If PrevSet = True Then
        ...
        DeleteSetting PlaceProgSettings, PlaceForm1, "Print1"
    End If
    ...
    SaveSetting PlaceProgSettings, PlaceForm1, "Print1", Me.chPrint
    
'------------------------------Start PostgreSQL----------------------------------
    Set cn = New ADODB.Connection
        cn.ConnectionTimeout = 10
        cn.Open ConStr
    Set rs = cn.Execute("UPDATE settings_soft SET value = '" & Me.txtNumSheets.Text & "' WHERE parameter = 'NumSheetsForm1'")
    rs.Close
    Set rs = Nothing
    cn.Close
    MousePointer = vbDefault
