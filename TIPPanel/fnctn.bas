...
'променливи за настойка на силозите
...
Public uniDailyBalance      As String
...
Public Dispatcher           As Boolean 'флаг за стартиран диспечер
...
Public Declare Function BitBlt _
                ...
                ByVal XSrc As Integer, ByVal YSrc As Integer, ByVal dwRop As Long) As Integer
...
'        Else
            ...
            MsgRestart = "Рестартирайте програмата!"
            ...
            frmDispPanelCap = "Машина " & MachineNumber & " - ТИП-Панел v" & App.Major & "." & App.Minor
...
Public Function strGetDesktopPath() As String
    'функция за търсене на път на декстопа
...
Public Sub AutoColW(ListViewTemp As MSComctlLib.ListView)
...
Public Function PrintLVPic(lvw As ListView, _
    ...
    Dim list_item       As MSComctlLib.ListItem
    Dim i               As Integer
    ...
    Line = 1
Again:
    If frmPrint.barPrint.Value < frmPrint.barPrint.Max Then
...
Private Sub ScalePicMix(ByVal Percentage As Single, _
    ...
    If frmPrintMix.barPrint.Value < frmPrintMix.barPrint.Max Then
        frmPrintMix.barPrint.Value = frmPrintMix.barPrint.Value + 10
    Else
    End If
    Percentage = Percentage / 100
...
Public Function GetStat()
    'функция за следене на статуса на машината през OPC сървъра от адрес CIO1000, CIO1001, CIO1002, CIO1003, CIO1004
    
    Dim StatNow     As String
    Dim StatNow1    As String
    Dim StatNow2    As String
    Dim StatNow3    As String
    Dim StatNow4    As String
    Dim rfornreq    As Integer
    Dim streq       As Integer
    Dim finreq      As Integer
    Dim mixmix      As Boolean
    Dim iauto       As Integer
    Dim iman        As Integer
    Dim iavar       As Integer
    Dim iopened     As Integer
    Dim iavaria     As Integer
    Dim avstop      As Integer
    Dim stLenta     As Integer
    Dim stValve2    As Integer
    Dim stValve3    As Integer
    Dim stValve4    As Integer
    Dim i           As Integer
    
    rfornreq = -1 'готов за заявка
    streq = -1 'стартирана заявка
    finreq = -1 'завършена заявка
    mixmix = False ' миксер 1-вкл, 0-изкл.
    iauto = -1 'автоматичен режим
    iman = -1 'ръчен режим
    iavar = -1 'авариен режим
    iopened = -1 '1-шибър отворен, 0-шибър затворен
    iavaria = -1 'авария
    avstop = -1 'авариен стоп
    stLenta = -1 '1-лента работи, 0-не работи
    stValve2 = -1 '1-клапа цимент отворена, 0-затворена
    stValve3 = -1 '1-клапа вода отворена, 0-затворена
    stValve4 = -1 '1-клапа хд отворена, 0-затворена
    ...
    Else
        If Val(frmOPC.cio1000.ItemValue) <> 0 Then
            StatNow = DecToBin(frmOPC.cio1000.ItemValue, 16)
            rfornreq = Mid$(StatNow, 16, 1)
            streq = Mid$(StatNow, 15, 1)
            finreq = Mid$(StatNow, 14, 1)
            iauto = Mid$(StatNow, 11, 1)
            iman = Mid$(StatNow, 10, 1)
            iavar = Mid$(StatNow, 9, 1)
            iopened = Mid$(StatNow, 4, 1)
            iavaria = Mid$(StatNow, 2, 1)
            avstop = Mid$(StatNow, 1, 1)
        End If
        DispPanel.indMode.Caption = ""
        DispPanel.indAvaria.Caption = ""
        DispPanel.indReq.Caption = ""
        ...
        For i = 0 To 15
            ...
            End If
            If iavar = 1 Then
                DispPanel.indMode.Caption = statAv
                DispPanel.indMode.ForeColor = &HFF&
            Else
            End If
            If iavaria = 1 Then
            ...
            If streq = 1 Then
                ...
                DispPanel.indReq.ForeColor = &HFF&
                DispPanel.btnDispStart.Enabled = False
            ElseIf streq = 0 And frmLogin.AdminSuccess = False Then
                DispPanel.btnDispStart.Enabled = True
            End If
            If rfornreq = 1 Then
                DispPanel.indReq.Caption = statReadyReqNew
                DispPanel.indReq.ForeColor = &HC000&
            Else
            End If
            If iopened = 1 Then
                DispPanel.indValveMix.Caption = statMixOpened
                DispPanel.indValveMix.ForeColor = &HC000&
            ElseIf iopened = 0 Then
                DispPanel.indValveMix.Caption = statMixClosed
                DispPanel.indValveMix.ForeColor = &HFF&
            End If
        Next i
        If Val(frmOPC.cio1001.ItemValue) <> -1 Then
            StatNow1 = DecToBin(frmOPC.cio1001.ItemValue, 16)
            stLenta = Mid$(StatNow1, 5, 1)
        End If
        If Val(frmOPC.cio1003.ItemValue) <> -1 Then
            StatNow2 = DecToBin(frmOPC.cio1003.ItemValue, 16)
            stValve2 = Mid$(StatNow2, 5, 1)
        End If
  
        If Val(frmOPC.cio1002.ItemValue) <> -1 Then
            StatNow3 = DecToBin(frmOPC.cio1002.ItemValue, 16)
            stValve3 = Mid$(StatNow3, 5, 1)
        End If

        If Val(frmOPC.cio1004.ItemValue) <> -1 Then
            StatNow4 = DecToBin(frmOPC.cio1004.ItemValue, 16)
            stValve4 = Mid$(StatNow4, 5, 1)
        End If
    End If
End Function
...
Public Function MaintConn() As Boolean
    ...
    If frmOPC.my12.ItemValue = 3333 And Val(frmOPC.cio1000.ItemValue) <> 0 Then
...
Public Function SendToController()
    'функция за изпращане на данните към контролера
    ...
    Dim KgScrVal    As Long
    Dim KgWatVal    As Long
    Dim KgChemVal   As Long

    Dim hexIM       As String

    Dim hexIM1      As String

    Dim hexIM2      As String

    Dim hexScr      As String

    Dim hexScr1     As String

    Dim hexScr2     As String

    Dim hexWat      As String

    Dim hexWat1     As String

    Dim hexWat2     As String

    Dim hexChem     As String

    Dim hexChem1    As String

    Dim hexChem2    As String

    Dim decTimeMix  As String

    Dim decTimePour As String

    Dim nQuant      As Single

    Dim tAggr       As Integer

    Dim tCem        As Integer

    Dim tWat        As Integer

    Dim tChem       As Integer
    Dim da          As Long
    Dim KgIMVal1    As Integer
    Dim KgIMVal2    As Integer
    Dim KgScrVal1   As Integer
    Dim KgScrVal2   As Integer
    Dim KgWatVal1   As Integer
    Dim KgWatVal2   As Integer
    Dim KgChemVal1  As Integer
    Dim KgChemVal2  As Integer
    ...
    For i = 1 To ns1
        ...
        SendRec.IMkg(i) = Val(DispConfirm.txtConfRecKg1(i - 1).Text)
    Next i

    For i = 1 To ns3
        SendRec.SCRname(i) = DispConfirm.txtConfRec3(i - 1).Text
        SendRec.SCRkg(i) = Val(DispConfirm.txtConfRecKg3(i - 1).Text)
    Next i

    For i = 1 To ns2
        SendRec.WATname(i) = Wat(i)
        SendRec.WATkg(i) = Val(DispConfirm.txtConfRecKg2(i - 1).Text)
    Next i

    For i = 1 To ns4
        SendRec.CHEMname(i) = DispConfirm.txtConfRec4(i - 1).Text
        SendRec.CHEMkg(i) = CSng(rDs(DispConfirm.txtConfRecKg4(i - 1).Text))
    Next i
    ...
    SendRec.Tpour = Val(DispConfirm.txtConfRecTimePour.Text) * 10
    SendRec.Tmix = Val(DispConfirm.txtConfRecTimeMix.Text) * 10
...
SendingData:
    ...
    For i = 0 To 49
        frmOPC.dm1000(i).ItemValue = 0
    Next i

    Last = Last + 1 'брояч на последната запълнена клетка на OPC
    da = CLng(cyc)
    frmOPC.dm1000(0).ItemValue = CLng(cyc) 'зареждаме брой цикли
    
    For i = 1 To ns1 'зареждаме инертните материали

        If Val(DispConfirm.initIM(i - 1)) > 0 And SendRec.IMkg(i) > 0 Then

            Select Case DispConfirm.initIM(i - 1) 'зареждаме инициализатори на течките от контролера

                Case 11
                    tAggr = frmOPC.dm11.ItemValue

                Case 12
                    tAggr = frmOPC.dm12.ItemValue

                Case 13
                    tAggr = frmOPC.dm13.ItemValue

                Case 14
                    tAggr = frmOPC.dm14.ItemValue

                Case 15
                    tAggr = frmOPC.dm15.ItemValue

                    '                Case 16
                    '                    tAggr = frmOPC.dm16.ItemValue
                Case Else
                    Screen.MousePointer = vbDefault
            ...
            If tAggr <= 0 Then tAggr = DispConfirm.initIM(i - 1) 'ако липсва инициализатор, то вземаме стандарните (11,12,13,14,15)
            frmOPC.dm1000(Last).ItemValue = tAggr 'зареждаме инициалиратор на течка в областа за рецепта в контролера
            KgIMVal = ToIEEE754(SendRec.IMkg(i)) 'преобразуваме в IEEE754
            hexIM = Hex(KgIMVal) 'минаваме в шестнадесетичен режим
            hexIM1 = Mid$(hexIM, 5, 8) 'разделяме числото на два стринга
            hexIM2 = Mid$(hexIM, 1, 4)
            KgIMVal1 = CInt("&H" & hexIM1) 'връщаме по отделно двата стринга в десетичен режим
            KgIMVal2 = CInt("&H" & hexIM2)
            frmOPC.dm1000(Last + 1).ItemValue = KgIMVal1 'зареждаме резултата в областа за рецепта на контролера
            frmOPC.dm1000(Last + 2).ItemValue = KgIMVal2
            Last = Last + 3 'преместваме брояча към следващата клетка в областа за рецепта
        Else
        End If
    ...
    For i = 1 To ns3 'зареждаме цимент - аналогично на инерния материал
        ...
        If Val(DispConfirm.initCem(i - 1)) > 0 And SendRec.SCRkg(i) > 0 Then

            Select Case DispConfirm.initCem(i - 1)

                Case 31
                    tCem = frmOPC.dm31.ItemValue

                Case 32
                    tCem = frmOPC.dm32.ItemValue

                Case 33
                    tCem = frmOPC.dm33.ItemValue

                Case 34
                    tCem = frmOPC.dm34.ItemValue
            ...
            If tCem <= 0 Then tCem = DispConfirm.initCem(i - 1)
            frmOPC.dm1000(Last).ItemValue = tCem
            KgScrVal = ToIEEE754(SendRec.SCRkg(i))
            hexScr = Hex(KgScrVal)
            hexScr1 = Mid$(hexScr, 5, 8)
            hexScr2 = Mid$(hexScr, 1, 4)
            KgScrVal1 = CInt("&H" & hexScr1)
            KgScrVal2 = CInt("&H" & hexScr2)
            frmOPC.dm1000(Last + 1).ItemValue = KgScrVal1
            frmOPC.dm1000(Last + 2).ItemValue = KgScrVal2
            Last = Last + 3
        Else
        End If
    ...
    'зареждаме вода - аналогично на инерния материал
    For i = 1 To ns2

        If Val(DispConfirm.initWat(i - 1)) > 0 And SendRec.WATkg(i) > 0 Then

            Select Case DispConfirm.initWat(i - 1)

                Case 21
                    tWat = frmOPC.dm21.ItemValue

                    '                Case 22
                    '                    tWat = frmOPC.dm22.ItemValue
                Case Else
                    Screen.MousePointer = vbDefault
            ...
            If tWat = 0 Then tWat = DispConfirm.initWat(i - 1)
            frmOPC.dm1000(Last).ItemValue = tWat
            KgWatVal = ToIEEE754(SendRec.WATkg(1))
            hexWat = Hex(KgWatVal)
            hexWat1 = Mid$(hexWat, 5, 8)
            hexWat2 = Mid$(hexWat, 1, 4)
            KgWatVal1 = CInt("&H" & hexWat1)
            KgWatVal2 = CInt("&H" & hexWat2)
            frmOPC.dm1000(Last + 1).ItemValue = KgWatVal1
            frmOPC.dm1000(Last + 2).ItemValue = KgWatVal2
            Last = Last + 3
        Else
        End If
    ...
    For i = 1 To ns4 'зареждаме химия - аналогично на инерния материал
        ...
        If Val(DispConfirm.initChem(i - 1)) > 0 And SendRec.CHEMkg(i) > 0 Then

            Select Case DispConfirm.initChem(i - 1)

                Case 41
                    tChem = frmOPC.dm41.ItemValue

                Case 42
                    tChem = frmOPC.dm42.ItemValue

                Case 43
                    tChem = frmOPC.dm43.ItemValue

                Case 44
                    tChem = frmOPC.dm44.ItemValue

                Case 45
                    tChem = frmOPC.dm45.ItemValue

                Case 46
                    tChem = frmOPC.dm46.ItemValue
            ...
            If tChem <= 0 Then tChem = DispConfirm.initChem(i - 1)
            frmOPC.dm1000(Last).ItemValue = tChem
            KgChemVal = ToIEEE754(SendRec.CHEMkg(i))
            hexChem = Hex(KgChemVal)
            hexChem1 = Mid$(hexChem, 5, 8)
            hexChem2 = Mid$(hexChem, 1, 4)
            KgChemVal1 = CInt("&H" & hexChem1)
            KgChemVal2 = CInt("&H" & hexChem2)
            frmOPC.dm1000(Last + 1).ItemValue = KgChemVal1
            frmOPC.dm1000(Last + 2).ItemValue = KgChemVal2
            Last = Last + 3
        Else
        End If
    ...
    'отместваме брояча на място за зареждане на времената
    Last = Last + 9
    
    'изчисляваме времената и ги зареждаме в контролера
    
    Screen.MousePointer = vbHourglass
    
    decTimePour = CInt("&H" & SendRec.Tpour)
    frmOPC.dm1000(Last).ItemValue = decTimePour
    Last = Last + 1
    decTimeMix = CInt("&H" & SendRec.Tmix)
    frmOPC.dm1000(Last).ItemValue = decTimeMix
    ...
    If Recs > 0 Then
        ...
        num = CInt(Recs)
        sss = num
        ssss = num & "-" & Left$(RecClasss, 7 - Len(sss))
        frmOPC.dm1070.ItemValue = ssss
        frmOPC.cio1005.ItemValue = 1
        frmOPC.dm1070.ItemValue = ssss
...
' Messageshow
        Screen.MousePointer = vbDefault
        MsgBox MsgRestart, vbCritical Or vbYesOnly
    End If
...
Public Function GetResult()
    ...
    If CInt(frmOPC.dm1100(0).ItemValue) = HelpRes And Recs > 0 Then
        ...
        'разчитане на замеса-----------------------------------------------------------------
        For b = 1 To 49
            hexdm(b) = Hex$(frmOPC.dm1100(b).ItemValue)
        ...
        For e = 1 To ns1
            ...
            ttt = (hexdm(3 * e) & hexdm(3 * e - 1))
            ResNow.IMmeasured(e) = ARound(IEEE754(CLng("&H" & ttt)), 0)  'кг по измерено им
        ...
        For e = 1 To ns3
            ...
            ttt = (hexdm(3 * (e + ns1 + 1)) & hexdm(3 * (e + ns1 + 1) - 1))
            ResNow.SCRmeasured(e) = ARound(IEEE754(CLng("&H" & ttt)), 0) 'кг по измерено цимент
        ...
        For e = 1 To ns2
            ...
            ttt = (hexdm(3 * (ns1 + e)) & hexdm(3 * (ns1 + e) - 1))
            ResNow.WATmeasured(e) = ARound(IEEE754(CLng("&H" & ttt)), 0) 'кг по измерено вода
        ...
        For e = 1 To ns4
            ...
            ttt = (hexdm(3 * (e + ns1 + ns3 + ns2)) & hexdm(3 * (e + ns1 + ns3 + ns2) - 1))
            ResNow.CHEMmeasured(e) = ARound(IEEE754(CLng("&H" & ttt)), 2)  'кг по измерено химия
...
'EmptyData = True
        ...
        If EmptyData = False Then
            ...
            Set rsUnique = cnUnique.Execute("UPDATE orders SET order_qmade = '" & ARound(CSng(rDs(rsUnique!order_qmade)) + CSng(rDs(ResNow.TotalQuant)), 3) & "' WHERE order_num =" & ResNow.OrderCode & ";")
        Else
        ...
        If cyc = HelpRes Then
            ...
            If EmptyData = True Then
                Dim response As Integer
                'бг
                response = MsgBox(MsgCorData, vbYesNo Or vbQuestion, "Липсват данни")
...
Public Function OpenRecepies()
    ...
    Do While Not rs.EOF
        ...
        End If
        If Not frCheck Then
            DispPanel.txtRec.Text = Format(RecCount + 1, "0000")
        End If
...
Public Function SvNwRecBut()
    ...
    If DispPanel.frRecepies.Enabled = True And DispPanel.frRecepies.Visible = True Then
        If Val(DispPanel.txtRec.Text) = 0 Then
...
Public Function OpenMaterials()
    ...
    DispPanel.txtMatHum.TabIndex = 2
    DispPanel.s1(0).TabIndex = 3
    DispPanel.s1(1).TabIndex = 4
    DispPanel.s1(2).TabIndex = 5
    DispPanel.s1(3).TabIndex = 6
    DispPanel.s1(4).TabIndex = 7
    DispPanel.s1(5).TabIndex = 8
    DispPanel.btnClearMat.TabIndex = 9
    DispPanel.btnSvNwMat.TabIndex = 10
    DispPanel.btnDelMat.TabIndex = 11
    DispPanel.btnAddMatDlvr.TabIndex = 12
    DispPanel.btnSvExp.TabIndex = 13
    DispPanel.btnDisp.TabIndex = 14
    DispPanel.btnOrders.TabIndex = 15
    DispPanel.btnRecepies.TabIndex = 16
    DispPanel.btnClients.TabIndex = 17
    DispPanel.btnDrivers.TabIndex = 18
    DispPanel.btnSuppliers.TabIndex = 19
    DispPanel.btnMaterials.TabIndex = 20
    DispPanel.btnNotes.TabIndex = 21
    DispPanel.btnAdminPanel.TabIndex = 22
    DispPanel.btnExit.TabIndex = 23
    ...
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniLoad & " М-1"
    colx.Width = 2000
    
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniLoad & " М-2"
    colx.Width = 2000
    ...
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniSold & " М-1"
    colx.Width = 1500
    
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniSold & " М-2"
    colx.Width = 1500
    ...
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniHumidity & " М-1"
    colx.Width = 500
    
    Set colx = DispPanel.lstMat.ColumnHeaders.Add()
    colx.Text = uniHumidity & " М-2"
    colx.Width = 500
    ...
    DispPanel.txtMatHum.MaxLength = 4
    ...
    Do While Not rs.EOF Or Not rsOther.EOF
        ...
        If MachineNumber = 1 Then
            If Val(rs!m_type) = 0 Then
                If rs!m_humidity <> Null Then
                    itmX.SubItems(9) = rDs(rs!m_humidity)
                Else
                    itmX.SubItems(9) = "0 %"
                End If
                
                If rsOther!m_humidity <> Null Then
                    itmX.SubItems(10) = rDs(rsOther!m_humidity)
                Else
                    itmX.SubItems(10) = "0 %"
                End If
            Else
            ...
            End If
        ElseIf MachineNumber = 2 Then
            If Val(rsOther!m_type) = 0 Then
                If rsOther!m_humidity <> Null Then
                    itmX.SubItems(9) = rDs(rsOther!m_humidity)
                Else
                    itmX.SubItems(9) = "0 %"
                End If
                
                If rs!m_humidity <> Null Then
                    itmX.SubItems(10) = rDs(rs!m_humidity)
                Else
                    itmX.SubItems(10) = "0 %"
                End If
            Else
    ...
    If DispPanel.frMaterials.Enabled = True And DispPanel.frMaterials.Visible = True Then
        ...
        If Len(DispPanel.txtMat.Text) > 0 And Len(DispPanel.txtMatName.Text) > 0 And Len(DispPanel.cmbMatType.Text) > 0 Then
            ...
            If MatNew.Kind = 0 Then 'им
                ...
                MatNotL = " 0 0 0 0 0 0"
                MatNew.Humidity = Val(rDs(DispPanel.txtMatHum.Text))
            ...
            If numCheck <> Val(DispPanel.txtMat) And nameCheck <> DispPanel.txtMatName And nameCheck1 <> DispPanel.txtMatName Then
                ...
                comIns = "INSERT INTO materials_bc" & MachineNumber & " VALUES(" & MatNew.Code & ",'" & MatNew.Title & "','" & MatNew.Kind & "','" & MatNew.Loaded & "','" & MatNew.Delivered & "','" & MatNew.Sold & "','" & MatNew.Humidity & "')"
                Set rs = cn.Execute(comIns)
                comIns = "INSERT INTO materials_bc" & MachineOther & " VALUES(" & MatNew.Code & ",'" & MatNew.Title & "','" & MatNew.Kind & "','" & MatNotL & "','" & MatNew.Delivered & "','" & MatNew.Sold & "','" & MatNew.Humidity & "')"
...
Public Function LoadMat()
    ...
    DispPanel.txtMatHum.Visible = False
    DispPanel.txtMatHum.Text = ""
    DispPanel.lblMatHum.Visible = False
    
    For i = 0 To 5
        DispPanel.s1(i).Visible = False
        DispPanel.s1(i).Value = 0
    Next i
    ...
    If DispPanel.cmbMatType.ListIndex > -1 Then
        If DispPanel.cmbMatType.Text = uniIM Then
            ...
            DispPanel.txtMatHum.Visible = True
            DispPanel.lblMatHum.Visible = True
            DispPanel.btnSvExp.Visible = False
        Else
        End If
...
Public Function StatBarRefresh()
        DispPanel.StatusBar.Panels(3).AutoSize = sbrContents
        DispPanel.StatusBar.Panels(4).AutoSize = sbrContents
        DispPanel.StatusBar.Panels(5).AutoSize = sbrContents
        DispPanel.StatusBar.Panels(6).AutoSize = sbrContents
        DispPanel.StatusBar.Panels(7).AutoSize = sbrContents
End Function
